<?php
	...
	$btnProcessCallClicked = ["btnProcessCall"];
	if($btnDispatchClicked == false && $btnProcessCallClicked == false){
		header("location: logcall.php");
	}
	
	$sql = "SELECT incident.caller_name, incident.phone_number, incident_type.incident_type_desc, incident.incident_locations,
	incident.incident_desc, incident.time_called, incident_status.incident_status_desc from incident inner join incident_type on 
	incident.incident_type_id = incident_type.incident_type_id inner join incident_status on 
	incident.incident_status_id = incident_status.incident_status_id where incident.incident_desc like " . "'%" . $descriptionOfIncident . "%'";
	...
	while($row = $result->fetch_assoc()){
		...
		$location = $row["incident_locations"];
		$incidentDesc = $row["incident_desc"];
		$time_called = $row["time_called"];
		$status = $row["incident_status_desc"];
		$incident = ["caller_name"=>$caller_name, "phone_number"=>$phone_number, "incident_type"=>$incident_type, "location"=>$location,
		"incidentDesc"=>$incidentDesc, "time_called"=>$time_called, "status"=>$status];
	...
	if($btnDispatchClicked == true){
		...
		if($hasCarSelection){
			...
			$numOfPatrolCarDispatch = count($patrolcarDispatched);
		}
		...
		`incident_locations`, `incident_desc`, `incident_status_id`, `time_called`) 
		VALUES ('" . $callerName . "','" . $contactNo . "','" . 
		$typeOfIncident . "','" . $locationOfIncident . "','" . $descriptionOfIncident . 
		"','" . $incidentStatus . "',now())";
		echo $sql;
		$conn = new mysqli(DB_SERVER,DB_USER,DB_PASSWORD,DB_DATABASE);
			...
			<div class="form-group row">
				...
				<div class="col-sm-8">
					<span>
						...
						?>
						<input name="descriptionOfIncident" type="hidden" rows="5" id="descriptionOfIncident" value="<?php echo $descriptionOfIncident;?>">
					</span>
				</div>
			</div>
			<div class="form-group row">
